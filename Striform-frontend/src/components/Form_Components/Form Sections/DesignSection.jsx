...
const DesignSection = () => {
    ...
    const handleColorChange = (color, index) => {
        ...
        setCurrentTheme('custom');
        console.log("Updating design in store:", newCustomTheme);
        updateFormDesign(formId, newCustomTheme); // Update the store with the new custom theme
    };
    ...
    return (
        <div className={"fixed top-0 right-0 w-[30vw] h-[100vh] bg-white shadow-lg p-4 z-50"}>
            <div>
            ...
            <div className={"text-sm font-semibold"}>
                <p>Font</p>
                <select className={"w-[90%] border border-black p-2 rounded-md outline-none"}>
                    {fonts.map((font, index) => (
                        <option className={"p-2"} key={index} value={font}>{font}</option>
                    ))}
                </select>
            </div>
            <input type={"file"} placeholder={"Select Background image"}/>
        </div>
    );
};
